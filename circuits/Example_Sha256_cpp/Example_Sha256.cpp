...
void Bits2Num_100_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void Bits2Num_100_run(uint ctx_index,Circom_CalcWit* ctx);
void Example_Sha256_101_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void Example_Sha256_101_run(uint ctx_index,Circom_CalcWit* ctx);
...
uint get_total_signal_no() {return 204685;}

uint get_number_of_components() {return 1813;}

uint get_size_of_input_hashmap() {return 256;}

uint get_size_of_witness() {return 28801;}
...
Fr_lt(&expaux[0],&lvar[1040],&circuitConstants[31]); // line circom 59
...
Fr_lt(&expaux[0],&lvar[1106],&circuitConstants[31]); // line circom 106
while(Fr_isTrue(&expaux[0])){
{
PFrElement aux_dest = &lvar[1107];
// load src
...
// end copying argument 0
// copying argument 1
Fr_copy(&lvarcall[1],&circuitConstants[36]);
// end copying argument 1
rrot_8(ctx,lvarcall,myId,&lvar[2],1);
// end call bucket
}
...
// load src
Fr_shr(&expaux[1],&signalValues[mySignalStart + 80],&lvar[3]); // line circom 32
...
Fr_eq(&expaux[0],&lvar[1],&signalValues[mySignalStart + 80]); // line circom 38
...
Fr_lt(&expaux[0],&lvar[256],&circuitConstants[31]); // line circom 68
while(Fr_isTrue(&expaux[0])){
{
PFrElement aux_dest = &lvar[256];
// load src
...
uint aux_create = 8;
int aux_cmp_num = 8+ctx_index+1;
uint csoffset = mySignalStart+1104;
for (uint i = 0; i < 1; i++) {
...
while(Fr_isTrue(&expaux[0])){
{
PFrElement aux_dest = &signalValues[mySignalStart + ((1 * Fr_toInt(&lvar[2])) + 336)];
// load src
// end load src
...
Fr_lt(&expaux[0],&lvar[2],&circuitConstants[138]); // line circom 29
while(Fr_isTrue(&expaux[0])){
Fr_sub(&expaux[1],&circuitConstants[152],&lvar[2]); // line circom 30
Fr_sub(&expaux[0],&expaux[1],&circuitConstants[2]); // line circom 30
{
PFrElement aux_dest = &signalValues[mySignalStart + ((1 * Fr_toInt(&expaux[0])) + 336)];
// load src
...
Fr_lt(&expaux[0],&lvar[2],&circuitConstants[152]); // line circom 72
while(Fr_isTrue(&expaux[0])){
{
uint cmp_index_ref = 8;
{
...
// load src
// end load src
Fr_copy(aux_dest,&signalValues[mySignalStart + ((1 * (0 + Fr_toInt(&lvar[2]))) + 336)]);
}
// run sub component if needed
if(!(ctx->componentMemory[mySubcomponents[cmp_index_ref]].inputCounter -= 1)){
Sha256compression_98_run(mySubcomponents[cmp_index_ref],ctx);
...
Fr_lt(&expaux[0],&lvar[2],&circuitConstants[137]); // line circom 77
while(Fr_isTrue(&expaux[0])){
{
PFrElement aux_dest = &signalValues[mySignalStart + ((1 * Fr_toInt(&lvar[2])) + 0)];
// load src
...
Fr_lt(&expaux[0],&lvar[3],&circuitConstants[137]); // line circom 61
while(Fr_isTrue(&expaux[0])){
{
PFrElement aux_dest = &lvar[1];
// load src
...
ctx->componentMemory[coffset].templateName = "Example_Sha256";
ctx->componentMemory[coffset].signalStart = soffset;
ctx->componentMemory[coffset].inputCounter = 1;
ctx->componentMemory[coffset].componentName = componentName;
ctx->componentMemory[coffset].idFather = componentFather;
...
for (uint i = 0; i < 1; i++) {
std::string new_cmp_name = "inBits";
...
uint aux_create = 1;
int aux_cmp_num = 2+ctx_index+1;
uint csoffset = mySignalStart+340;
for (uint i = 0; i < 1; i++) {
...
for (uint i = 0; i < 1; i++) {
std::string new_cmp_name = "outNum";
...
Fr_lt(&expaux[0],&lvar[1],&circuitConstants[0]); // line circom 13
while(Fr_isTrue(&expaux[0])){
{
uint cmp_index_ref = 1;
{
...
Fr_lt(&expaux[0],&lvar[1],&circuitConstants[137]); // line circom 18
while(Fr_isTrue(&expaux[0])){
{
uint cmp_index_ref = 2;
{
...
// run sub component if needed
if(!(ctx->componentMemory[mySubcomponents[cmp_index_ref]].inputCounter -= 1)){
Bits2Num_100_run(mySubcomponents[cmp_index_ref],ctx);
...
Example_Sha256_101_create(1,0,ctx,"main",0);
Example_Sha256_101_run(0,ctx);
}
